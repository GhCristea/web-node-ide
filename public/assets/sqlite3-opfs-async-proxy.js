...
;(function () {
  ...
  const installAsyncProxy = function () {
    ...
    state.verbose = 1
    const loggers = { 0: console.error.bind(console), 1: console.warn.bind(console), 2: console.log.bind(console) }
    ...
    class GetSyncHandleError extends Error {
      constructor(errorObject, ...msg) {
        super([...msg, ': ' + errorObject.name + ':', errorObject.message].join(' '), { cause: errorObject })
      ...
      xOpen: async function (fid, filename, flags, opfsFlags) {
        ...
        try {
          ...
          fh.releaseImplicitLocks =
            opfsFlags & state.opfsFlags.OPFS_UNLOCK_ASAP || state.opfsFlags.defaultUnlockAsap
          __openFiles[fid] = fh
    ...
    const initS11n = () => {
      ...
      state.s11n.storeException = state.asyncS11nExceptions
        ? (priority, e) => {
            if (priority <= state.asyncS11nExceptions) state.s11n.serialize([e.name, ': ', e.message].join(''))
          }
        : () => {}
      return state.s11n
    }
