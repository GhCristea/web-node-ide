...
import type { FileNode } from './service/types';
...
/**
 * Converts a flat path map (from generateFilePaths) into a nested FileSystemTree
 * expected by WebContainer.mount().
 */
export function buildWebContainerTree(files: Record<string, string>): FileSystemTree {
  ...
  for (const [path, content] of Object.entries(files)) {
    ...
    for (let i = 0; i < parts.length; i++) {
      ...
      if (isFile) {
        ...
        // @ts-expect-error entry is DirectoryNode because we created it above
