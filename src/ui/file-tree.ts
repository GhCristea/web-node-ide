...
export class FileTreeComponent {
  ...
  private tree: Map<string, boolean> = new Map()
  ...
  private async renderDirectory(path: string, container: HTMLElement, depth: number): Promise<void> {
    try {
      const entries = (await this.filesystem.readdir(path, { withFileTypes: true })) as Dirent[]
      ...
      console.error(`Failed to render directory ${path}`)
    }
  }

  private renderEntry(entry: Dirent, fullPath: string, container: HTMLElement, depth: number): void {
    ...
    if (isDir) {
      ...
      toggle.onclick = e => {
        e.stopPropagation()
        this.toggleDirectory(fullPath, isExpanded)
      }
  ...
  private toggleDirectory(path: string, isExpanded: boolean): void {
  ...
  private dispatchFileSelect(path: string): void {
    const event = new CustomEvent('file-select', { detail: { path }, bubbles: true })
